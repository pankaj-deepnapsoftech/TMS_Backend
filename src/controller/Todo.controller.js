...
export const GetAllTodo = AsyncHandler(async (req, res) => {
    ...
    const data = await TodoModel.find({}).sort({ _id: -1 }).skip(skip).limit(limits);
...
export const ChangeStatus = AsyncHandler(async (req,res) => {
    const data = req.body;
    const {id} = req.params;
    const todo = await TodoModel.findByIdAndUpdate(id,{$push:{statusHistory:{...data,changedBy:req?.user?._id}}},{new:true})
    if(!todo){
        return res.status(404).json({
            message:"Data not found"
        })
